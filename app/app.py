...
<style>
    ...
    .info-card {{
        background-color: {SECONDARY_LIGHT}; color: {PRIMARY}; border-radius: 10px;
        padding: 0.9rem 1.2rem; font-size: 0.95rem; border-left: 3px solid {SECONDARY};
    }}
    .success-card {{
        background-color: {SECONDARY_LIGHT}; color: {PRIMARY}; border-radius: 10px;
        padding: 0.9rem 1.2rem; font-size: 0.95rem; border-left: 3px solid {ACCENT};
        font-weight: 600;
    }}
    .result-card p {{ margin: 0 0 0.5rem 0; color: {TEXT}; font-weight: 600; font-size: 0.95rem; }}
    hr {{ background-color: {SECONDARY_LIGHT}; }}
    div[data-baseweb="select"] > div {{
        border-color: {SECONDARY_LIGHT} !important; border-radius: 8px !important;
    }}
...
# --------------------------------------------------------------------------
st.markdown(
    f'<div class="kicker" style="color:{ACCENT};font-weight:700;font-size:0.8rem;'
    f'letter-spacing:2px;text-transform:uppercase;">FASE 6 · PRODUCTIVIZACIÓN DEL MODELO</div>',
    unsafe_allow_html=True,
)
...
with col_resultado:
    ...
    if not calcular:
        st.markdown(
            '<div class="info-card">👈 Completa tu perfil a la izquierda y pulsa '
            '<strong>Recomendar rol</strong> para ver el resultado.</div>',
            unsafe_allow_html=True,
        )
    else:
        ...
        # Solo hay una serie (probabilidad por rol) y cada barra ya lleva su
        # nombre en el eje Y, así que el color no necesita distinguir
        # identidad entre roles: se usa para dar ÉNFASIS. El rol ganador va
        # en ámbar (el mismo tono que usamos para "cifra destacada" en toda
        # la memoria y las presentaciones) y el resto en un gris verdoso
        # apagado, para que el ojo vaya directo a la respuesta en vez de
        # perderse en siete colores sin significado.
        roles = [r for r, _ in ranking]
        probs = [p * 100 for _, p in ranking]
        colores = [ACCENT if i == 0 else MUTED for i in range(len(roles))]

        fig = go.Figure(go.Bar(
            x=probs[::-1], y=roles[::-1], orientation="h",
            marker=dict(color=colores[::-1], line=dict(width=0)),
            text=[f"{p:.1f}%" for p in probs[::-1]],
            textposition="outside",
            textfont=dict(color=TEXT, size=12, family="Arial, sans-serif"),
            hovertemplate="<b>%{y}</b><br>Probabilidad: %{x:.1f}%<extra></extra>",
        ))
        fig.update_layout(
            title=dict(
                text="Ranking completo de roles por probabilidad",
                font=dict(size=15, color=PRIMARY, family="Arial, sans-serif"),
            ),
            xaxis_title="Probabilidad (%)", xaxis_range=[0, max(probs) * 1.2],
            xaxis=dict(gridcolor=SECONDARY_LIGHT, zeroline=False),
            yaxis=dict(showgrid=False),
            plot_bgcolor="#FFFFFF", paper_bgcolor="#FFFFFF",
            font=dict(family="Arial, sans-serif", color=TEXT, size=13), height=320,
            bargap=0.35,
        ...
        if skills_que_faltan:
            ...
            st.markdown(
                f'<div class="result-card"><p>Para acercarte a {rol_top}, prioriza estas skills:</p>{chips}</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                '<div class="success-card">✓ Ya tienes todas las skills más demandadas para este rol.</div>',
                unsafe_allow_html=True,
            )
